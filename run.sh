...
#   ./run.sh demo | seed | build-facts | train | serve | test | install
set -euo pipefail
PY="${PY:-python3}"
...
case "${1:-demo}" in
  ...
  train)        $PY -m vi.ranker.train --db "$DB" ;;
  ...
  demo)         "$0" seed && "$0" build-facts && "$0" train && "$0" serve ;;
  clean)        rm -rf data/*.db data/*.db-* data/models/* ;;
  *)            echo "usage: $0 {demo|seed|build-facts|train|serve|test|install|clean}"; exit 1 ;;
esac
